...
class UrlHelper {
    ...
    public function __construct($url, $default, $languages = [], $pathPrefix = '') {
        $this->config = new UrlConfigHelper($url, $pathPrefix);
        $this->translate = new UrlTranslateHelper($default, $languages);
    }
    ...
    public function isTranslable() {
        ...
        foreach ($this->config->getExcludedUrls() as $regex) {
            $escapedRegex = TextHelper::escapeForRegex($regex);
    ...
    public function detectCurrentLanguage() {
        ...
        $escapedPathPrefix = TextHelper::escapeForRegex($this->config->getPathPrefix());
